...
Function InstallGstreamer
  ReadRegStr $0 HKLM 'SOFTWARE\WOW6432Node\GStreamer1.0\x86' InstallDir
  ReadRegStr $3 HKLM 'SOFTWARE\WOW6432Node\GStreamer1.0\x86_64' InstallDir
  ${If} $0 == ''
   ${AndIf} $3 == ''
     DetailPrint 'The vidlib depends on Gstreamer, we will install the Gstreamer runtime.'
     DetailPrint 'Downloading Gstreamer.'
     nsis_tauri_utils::download "https://gstreamer.freedesktop.org/data/pkg/windows/1.22.5/msvc/gstreamer-1.0-msvc-x86_64-1.22.5.msi" "$TEMP\gstreamer.msi"
     Pop $0
     ${If} $0 == 0
       DetailPrint "Download successful, installing."
       ExecWait 'msiexec /i "$TEMP\gstreamer.msi" /passive' $1
       ReadRegStr $0 HKLM 'SOFTWARE\WOW6432Node\GStreamer1.0\x86' InstallDir
       ReadRegStr $3 HKLM 'SOFTWARE\WOW6432Node\GStreamer1.0\x86_64' InstallDir
       ${If} $0 != ''
         StrCpy $GStreamerPath $0
       ${ElseIf} $3 != ''
         StrCpy $GStreamerPath $3
       ${Else}
         Abort "GStreamer installation failed."
       ${EndIf}
     ${Else}
       Abort "GStreamer download failed"
     ${EndIf}
  ${Else}
